...
import GenerationHistory, { GenerationRecord } from './GenerationHistory';
...
const SyntheticDataGenerator: React.FC = () => {
  ...
  const handleHistorySelect = (record: GenerationRecord) => {
  ...
  const handleHistoryDelete = (id: string) => {
    const savedHistory = localStorage.getItem('generationHistory');
    if (savedHistory) {
      const history = JSON.parse(savedHistory).filter((record: GenerationRecord) => record.id !== id);
      localStorage.setItem('generationHistory', JSON.stringify(history));
    }
  };
